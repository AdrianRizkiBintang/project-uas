<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-between align-center">
            <h2>Menu &mdash; {{ $outlet->name }}</h2>
            @php $cartCount = array_sum(array_column(session('cart', []), 'quantity')); @endphp
            <a href="{{ route('dine-in.cart') }}" class="btn btn-primary btn-sm">
                Keranjang
                @if($cartCount > 0)
...
<div class="container">
    ...
    <div class="card mb-20">
        <div class="card-body">
            <form method="GET" action="{{ route('dine-in.menu') }}" class="d-flex flex-wrap gap-8">
